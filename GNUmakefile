# GNUmakefile for simple-sftpd
# Requires GNU Make (gmake on FreeBSD). The top-level Makefile forwards to this file.
...
ifeq ($(PLATFORM),macos)
	...
	@echo "Installing build dependencies on FreeBSD..."
	sudo pkg install -y cmake gmake pkgconf git openssl jsoncpp yaml-cpp
else ifeq ($(PLATFORM),windows)
...
docker-run:
	docker run -d --name $(PROJECT_NAME)-$(VERSION) -p 21:21 $(PROJECT_NAME):$(VERSION)
...
ifeq ($(PLATFORM),macos)
	...
	@if exist scripts\build-windows.bat ( \
		...
		echo "  sc create SimpleSFTPDaemon binPath= \"$(INSTALL_PREFIX)\\bin\\$(PROJECT_NAME).exe\""; \
		echo "  sc start SimpleSFTPDaemon"; \
	)
endif
...
ifeq ($(PLATFORM),macos)
	...
	@sc query SimpleSFTPDaemon
endif

# Help - Main help (most common targets)
help:
	@echo "Simple Secure FTP Daemon - A secure, configurable, and feature-rich FTP server - Main Help"
	@echo "=================================="
	@echo ""
...
help-all:
	@echo "Simple Secure FTP Daemon - A secure, configurable, and feature-rich FTP server - All Available Targets"
	@echo "=============================================="
	@echo ""
	@echo "Essential targets:"
	@echo "  all              - Build the project (default)"
...
ifeq ($(PLATFORM),macos)
	...
	@sc config SimpleSFTPDaemon start= auto
endif
...
ifeq ($(PLATFORM),macos)
	...
	@sc config SimpleSFTPDaemon start= disabled
endif
...
ifeq ($(PLATFORM),macos)
	...
	@sc query SimpleSFTPDaemon >nul 2>&1 && ( \
		sc stop SimpleSFTPDaemon; \
		sc delete SimpleSFTPDaemon; \
		echo "Service uninstalled successfully"; \
	) || echo "Service not found"
endif
